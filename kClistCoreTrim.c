...
maximal k-clique densest subgraph is untouched.
  Feel free to use these lines as you wish.
  This program is adopted from "kClistCore.c" publicly available at
https://github.com/maxdan94/kClist.
  The theory behind this program is as follows. If some real number rho is a
lower bound on the density of the k-clique densest subgraph, then any node with
k-clique degree strictly smaller than rho cannot be in the k-clique densest
subgraph, and we can safely remove it. Recall that when computing the k-clique
core decomposition of a graph, we iteratively remove the node with the least k-
clique degree, resulting in a sequence of subgraphs of size n (the original
graph), n - 1, ..., 1. We take rho as the maximum k-clique density among these n
graphs (of course by examining more subgraphs we can have a better lower bound,
but that may take additional time) and revisit the elimination procedure that
computes the k-clique core decomposition, removing nodes until all the remaining
nodes has degree at least rho.

To compile:
  "gcc kClistCoreTrim.c -O3 -o kClistCoreTrim"

To execute:
  "./kClistCoreTrim k edgelist.txt ckdeg.txt ckcore.txt ckdens.txt tag"
  The parameters, except the last one, are the same as those in the original
code "kClistCore.c". In particular, k is the size of a clique considered, which
must be at least 3, while edgelist.txt is the name of the file that contains the
graph, in which each line contains one edge represented by two integers
separated by a space. Additionally, tag is a string specifying the dataset
(e.g., "dblp"), which is used to generate the output file name.
k of k-clique to enumerate.

Output:
  Apart from the information about the k-clique core decomposition and the
approximate k-clique densest subgraph in ckdeg.txt, ckcore.txt and ckdens.txt,
the program will also output
  - the trimmed graph, in the same format as edgelist.txt (one edge on each line
separated by a space);
  - the statistics, including the number of nodes, the number of edges, the
number of k-cliques in the trimmed graph, and the running time.
*/
...
typedef struct {
  ...
  unsigned ntrim; // Number of nodes in the trimmed graph
  unsigned long long ncktrim; // Number of k-cliques in the trimmed graph
} densest;
...
densest* kcliquecore(unsigned kmax,unsigned long long *nck, unsigned long long ncktot, sparse* g){
  ...
  unsigned long long *ckdegwhendel = malloc(g->n2 * sizeof(unsigned long long));
  ...
  for (i=0;i<g->n2-1;i++){
    ...
    size_tmp=g->n2-i-1;
    ncktot-= ckdegwhendel[size_tmp] = ncktot2/((unsigned long long)(kmax-1));//kcliques are counted kmax-1 times
  ...
  for (j = g->n2 - 1; j >= 1; --j) {
    if (ckdegwhendel[j] >= ds->ckrho) {
      ds->ntrim = j + 1;
      break;
    }
    ds->ncktrim -= ckdegwhendel[j];
  }
  kv=popminLLU(heap);
  ds->ic[i].key=kv.key;
  ds->ic[i].value=c;
...
void printdensest(densest *ds, sparse* g, char* ckcore, char* ckdens, char *trimg, char *stat, time_t t0){
  FILE *file;
  unsigned i;
  ...
  file = fopen(trimg, "w");
  char *alive = calloc(g->n2, sizeof(char));
  unsigned m = 0;
  for (i = g->n2 - 1; i != g->n2 - ds->ntrim - 1; --i) // Be careful!!! (unsigned)(-1) > 0 so "!=" is used here instead of ">"
    alive[ds->ic[i].key] = 1;
  for (i = 0; i < g->e2; ++i) {
    if (alive[g->edges[i].s] && alive[g->edges[i].t]) {
      fprintf(file, "%u %u\n", g->map[g->edges[i].s], g->map[g->edges[i].t]);
      ++m;
    }
  }
  free(alive);
  printf("The trimmed graph has %u nodes, %u edges and %llu k-cliques.\n", ds->ntrim, m, ds->ncktrim);
  fclose(file);

  file = fopen(stat, "w");
  fprintf(file, "[Number of Nodes]\t[Number of Edges]\t[Number of k-Cliques]\t[Time (seconds)]\n");
  fprintf(file, "%u\t%u\t%llu\t%ld\n", ds->ntrim, m, ds->ncktrim, time(NULL) - t0);
  fclose(file);
}
...
int main(int argc,char** argv){
  ...
  strcat(trim_file_name, argv[6]);
  strcat(trim_file_name, "_");
  strcat(trim_file_name, argv[1]);
  strcat(trim_file_name, ".txt");
  char stat_file_name[100] = "stat_trim_";
  strcat(stat_file_name, argv[6]);
  strcat(stat_file_name, "_");
  strcat(stat_file_name, argv[1]);
  strcat(stat_file_name, ".txt");
  printf("Writing the trimmed graph in file: %s\n", argv[6]);
  printdensest(ds,g,argv[4],argv[5], trim_file_name, stat_file_name, t0);
  freesparse(g);
  t2=time(NULL);
  printf("- Time = %ldh%ldm%lds\n",(t2-t1)/3600,((t2-t1)%3600)/60,((t2-t1)%60));
  t1=t2;
